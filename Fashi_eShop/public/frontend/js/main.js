...
function addCart(productId) {
    $.ajax({
        ...
        success: function (reponse) {
            ...
            $('.cart-price').text('$' + reponse['total']);
            $('.select-total h5').text('$' + reponse['total']);
            ...
            if (cartHover_existItem.length) {
                cartHover_existItem.find('.product-selected p').text('$' + reponse['cart'].price.toFixed(2) + 'x' + reponse['cart'].qty);
            } else {
                var newItem =
                    '<tr data-rowId="' + reponse['cart'].rowId + '">\n' +
                    '<td class="si-pic">\n' +
                    '<img src="/frontend/img/products/' + reponse['cart'].options.images[0].path + '" alt = "" >\n' +
                    '</td>\n' +
                    '<td class="si-text">\n' +
                    '<div class="product-selected">\n' +
                    '<p>$ ' + reponse['cart'].price.toFixed(2) + 'x' + reponse['cart'].qty + '</p>\n' +
                    '<h6>' + reponse['cart'].name + '</h6>\n' +
                    '</div>\n' +
                    '</td>\n' +
                    '<td class="si-close">\n' +
                    ' <i onclick="removeCart(\'' + reponse['cart'].rowId + '\')" class="ti-close"></i>\n' +
                    '</td>\n' +
                    '</tr>';
...
function removeCart(rowId) {
    $.ajax({
        ...
        // Xử lý nếu thành công
        success: function (reponse) {
            // Xử lý phần cart ở trang header
            $('.cart-count').text(reponse['count']);
            $('.cart-price').text('$' + reponse['total']);
            $('.select-total h5').text('$' + reponse['total']);
...
function destroyCart() {
    $.ajax({
        ...
        success: function (reponse) {
            ...
            // Thông báo
            alert('Xóa tất cả sản phẩm trong giỏ hàng thành công');
            console.log(reponse);
    ...
    $.ajax({
        ...
        success: function (reponse) {
            ...
            if(qty === 0) {
                ...
                cart_existItem.find('.total-price').text('$' + (reponse['cart'].price * reponse['cart'].qty).toFixed(2));
            }
            $('.subtotal span').text('$' + reponse['subtotal']);
            $('.cart-total span').text('$' + reponse['subtotal']);
            // Thông báo
